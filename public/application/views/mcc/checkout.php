...
<?			} ?>
			...
			<div id="checkout" class="grey-border left">
				<div class="left item3">
					...
					<br />
					<form method="post" action="https://<?=$_SERVER['SERVER_NAME']?>/shopping_cart/authorization" name="checkoutform" id="checkoutform">
          ...
          <input type="hidden" name="ordertotal" id="ordertotal" value="<?php echo ($order->subtotal+$order->shipping_total); ?>" />
		   	...
		   	<div class="form-element">
				...
				<input type="text" name="userEmail" id="userEmail" value="<?=$user ? $user->email : ''?>" />
			</div>
<?			if (!$user) { ?>
		   	<div class="form-element">
				Password:*
				<input type="password" name="userPassword" id="userPassword" value="" />
				<label class="comment">(At least 8 characters)</label>
			</div>
		   	<div class="form-element">
				Confirm Password:*
...
?>
			<div class="form-element">
				...
				<select name="<?=$field->formName?>" id="<?=$field->formName?>" style="width:155px;float:right;">
...
?>
          	<div class="form-element">
				<label for="<?=$field->formName?>"><?=$field->name?>:<?=$field->required?'*':''?></label>
				<input type="text" name="<?=$field->formName?>" value="<?=$field->value?>" />
<?				if ($field->comment != '') { ?>
				<label class="comment"><?=$field->comment?></label>
<?				} ?>
			</div>
<?
			}
...
?>
				...
				<div class="left  item3">
					...
					<br />
						...
						<div class="form-element">
							Same as Billing Address <input class="checkbox" type="checkbox" name="same" onClick="copyBillingInfo(this);" value="Shipping Address same as Billing Address" />
						</div>
<?		
...
?>
						<div class="form-element">
							Yes sign me up for email updates <input class="checkbox" type="checkbox" name="email-updates" value="Yes" />
						</div>

						<div id="share" class="grey-border">
							Share my creation with other users on MyChocolateCoins.com <input type="checkbox" name="share" value="Yes" checked />
						</div><!-- share -->
			
				</div><!-- shipping -->
				<div class="left item3">
					<center><img src="/env/images/mcc/step1.png" /></center>
...
?>
          ...
          <div id="shipping-methods">
               <center>
					<label>Please choose shipping method</label>
			...
			<div id="checkoutform_errorloc" class="error_strings" style="float:right;display:block;margin-top:80px;" class="right">
			 	
            </div>
			<span class="checking_email error_strings right" style="float:right;display:block;margin-top:80px; width: 230px; display:none;"></span>
			<div id="checkoutform_errorloc_loc" name="checkoutform_errorloc_loc" class="right" style="float:right;display:block;margin-top:80px;">
			</div>
			<div id="notice" class="right">
				<p style="text-align:right; font-weight:bold;">Custom products turnaround time is 5-7 business days plus shipping time<br>
				Grab and Go products turnaround time is 1 business day plus shipping time</p>
			</div>
			<div class="clear"></div>			
			<div id="request-date" class="right">
				...
				<input type="text" name="requesteddate" id="requesteddate" class="left" value="<?php echo $requesteddate; ?>" onFocus="if(getElementById('popupInfo').style.display=='block'){showhideBox('popupInfo');submitPressed = 0;}" >
				<img class="left" src="/env/images/calendar.png" onClick="javascript:$('#requesteddate').focus();" />
			</div><!-- request-date -->
			<div class="clear"></div>

			<!--<img src="/env/images/mcc/continue.png" alt="Submit" onClick="if(!checkDate()) return false; on_submit_validation();" style="margin-bottom: 30px;vertical-align:top;" class="right">-->
			<img src="/env/images/mcc/continue.png" alt="Submit" onClick="on_submit_validation();" style="margin-bottom: 30px;vertical-align:top;" class="right">
			
			
			<div id="popupInfo" style="width:500px;height:210px;position:absolute;background:#ffffff;border-style:solid;border-color:#ffcad2;bottom:-50px;margin:30px 0 0 60px;padding:10px;display:none;visibility:hidden;" onClick="showhideBox('popupInfo');">
				<div id="tooSoonHead"><H3>Rush Order</H3></div>
				<div class="clear pink-bar"></div>
				<div id="tooSoonText">
					<p>It might take up to 10 business days to deliver custom chocolate Product.</p>
					<p>If products are needed earlier, we offer RUSH DELIVERY. We will contact you if this is the case to process the RUSH ORDER FEE. RUSH ORDER FEE may be avoided by selecting a later 'Requested Date.' By clicking CONTINUE, you recognize that you may be contacted about the RUSH ORDER FEE.
					</p>
				</div>
				<div id="closeMe">Click to close and change Requested Date, or click Continue to accept that you may be contacted about a rush order fee.</div>
			</div>
...
<?	if (!$user || $user->id == 0) { ?>
	...
	$.getJSON('/customers/check_email/'+$('#userEmail').val(), function(data)
	{
		...
		if (data.status == 'OK') {
			...
			$('.checking_email').html('The email address is already registered. You may already have an account if you registered on our other site, MyChocolateHearts.com - if so you can use the same password to login above.');
		}
	});
	return;
<?	} else { ?>
