...
class Payment_Paypal_Driver implements Payment_Driver {
	...
	private $response = array("", "Approved", "Declined", "Error");
	...
	*/
	protected function set_express_checkout()
	{
		...
		if (strtoupper($response_array['ACK']) == 'SUCCESS')
		{
			...
			$database->query('UPDATE orders SET trans_id="'.$paypal_token.'" WHERE trans_id="'.$this->set_express_checkout_fields['INVNUM'].'"');
		...
		else // Something went terribly wrong...
		{
			...
			throw new Kohana_User_Exception('SetExpressCheckout ERROR', $response_array['L_LONGMESSAGE0']);
	...
	*/
	protected function get_express_checkout()
	{
		...
		else // Something went terribly wrong...
		{
			...
			throw new Kohana_User_Exception('SetExpressCheckout ERROR', $this->get_express_checkout_response['L_LONGMESSAGE0']);
	...
	*/
	protected function do_express_checkout_payment()
	{
		...
		if (strtoupper($this->nvp_response_array['ACK']) != 'SUCCESS')
		{
			...
			throw new Kohana_User_Exception('SetExpressCheckout ERROR', $this->nvp_response_array['L_LONGMESSAGE0']);
