...
class Orders_Controller extends My_Template_Controller {
	...
	public function view() {
		...
		$result = $db->query('SELECT orders.id, orders.trans_id, orders_baskets.*, products.name as productname FROM orders INNER JOIN orders_baskets ON orders.id = orders_baskets.order_id LEFT JOIN products ON orders_baskets.product_id = products.id WHERE orders.sessionID = \''.session_id().'\' AND orders.statusID IN (1,3)');
