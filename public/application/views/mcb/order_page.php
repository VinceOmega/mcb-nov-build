		...
		<div class="row col-md-12 col-xs-12 col-sm-12 col-lg-12 ghost">
			...
			<div class="col-md-3 col-xs-3 col-sm-3 col-lg-3 quantity-items">
			 <?php if(empty($order->designpath)): ?>
				<span class="cart-imgs"><img src="/env/product_images/<?php echo $order->productimage; ?>" alt="product" width="102"></span>
			<?php else: ?>
				<span class="cart-imgs"><a href="<?php echo $order->designpath; ?>" class="highslide" id="hsimage<?php echo $order->id; ?>" onclick="return hs.expand(this, { src: '<?php echo $order->designpath; ?>' })" border="0"><img src="<?php echo $order->designpath; ?>" alt="Your custom chocolate chocolate bar!" width="102" title="Click to enlarge your design" border="0" /></a></span>

			<?php endif; ?>
			</div>
			<div class="col-md-9 col-xs-9 col-sm-9 col-lg-9 quantity-description">
				<a href="#"><?php echo $order->productname; ?></a><br>
				<p> Product Unit Price : <b>US $<?php echo money_format('%.2n', $order->rate); ?></b></p><br>
				<a class="btn rnd cream" href="/products/show/<?php echo $order->title_url; ?>">Review Product</a>
			</div>
			...
			<!-- Brown Box -->	
				<div class="row col-md-4 col-xs-4 col-sm-4 col-lg-4 price-table">
					...
					&nbsp;&nbsp;$<?=money_format('%2n', $order->rate*50)?>-$<?=money_format('%2n', $order->rate*99)?>
					</div>
					...
					&nbsp;&nbsp;$<?=money_format('%2n', $order->rate*100)?>-$<?=money_format('%2n', $order->rate*249)?>
					</div>
					...
					&nbsp;&nbsp;$<?=money_format('%2n', $order->rate*250)?>-$<?=money_format('%2n', $order->rate*299)?>
					</div>
					...
					&nbsp;&nbsp;$<?=money_format('%2n', $order->rate*1000)?>
					</div>
				</div>
			<!-- Brown Box -->
			...
			<div class="col-md-4 col-xs-4 col-sm-4 col-lg-4 quantity-selector">
					...
					<input type="hidden" name="rate" class="item-rate" value="<?= $order->rate?>">
					<?php endforeach; ?>
					</span>
			</div>
				<div class="col-md-4 col-xs-4 col-sm-4 col-lg-4 quantity-tabulation">
						<p>Unit Price: <b>$<?=money_format('%2n', $order->rate)?></b></p>
